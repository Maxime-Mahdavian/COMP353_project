    ...
    case "show":
        ...
        function show ($cid, $rid, $name, $time, $message, $indent = 0) { ?>
            <div class="ccomment<?= $indent ? " creply" : "" ?>">
                ...
                <div style="font-size: 17px" class="cmessage"><?=$message?></div>
                <input type="button" class="cbutton ui smaller blue button" value="Reply" onclick="comments.reply(<?=$cid?>, <?=$rid?>)"/>
                <?php if($_SESSION['admin'] == 1)
                    echo '<input type="button" class="cbutton ui smaller red button" value="Delete" onclick="comments.del('.$cid.')"/>';
                ?>
